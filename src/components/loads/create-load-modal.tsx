...
import { MapPin, AlertCircle, Navigation, Search, Truck, Mail } from "lucide-react";
...
export function CreateLoadModal({ open, onOpenChange, onSuccess }: CreateLoadModalProps) {
  ...
  const pickedCarrierLabel = selectedCarrier
    ...
    ? `${carrierContactSelected.first_name || ""} ${carrierContactSelected.last_name || ""}`.trim() ||
      carrierContactSelected.email ||
      carrierContactSelected.phone
    : null;
  ...
  const handleCarrierSearch = (value: string) => {
    ...
    if (!value.trim()) { setCarrierResults([]); return; }
  ...
  const handleCarrierInvite = async () => {
    ...
    try {
      const { data } = await api.post<InviteResponse>("/users/invite", {
        contact: carrierContact.trim(),
        role: "carrier",
      });
                                ...
                                {carrierResults.map((c) => (
                                  ...
                                  >
                                    ...
                                    <p className="font-medium truncate flex-1">
                                      {c.alias || `${c.first_name || ""} ${c.last_name || ""}`.trim() || "Unknown"}
                                    </p>
                        ...
                        <TabsContent value="contact" className="mt-0 space-y-3">
                          <div className="flex gap-2">
                            <Input
                              ...
                              onChange={(e) => { setCarrierContact(e.target.value); setCarrierContactResult(null); setCarrierContactNotFound(false); setCarrierContactSelected(null); }}
                              onKeyDown={(e) => e.key === "Enter" && handleCarrierContactSearch()}
                            />
                            <Button type="button" variant="outline" onClick={handleCarrierContactSearch} disabled={!carrierContact.trim() || carrierContactSearching}>
                          ...
                          {carrierContactResult && !carrierContactSelected && (
                            <div className="flex items-center gap-3 rounded-lg border bg-muted/30 px-3 py-2.5">
                              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-primary text-xs font-semibold shrink-0 uppercase">
                                {carrierContactResult.first_name?.[0] || "?"}
                              </div>
                              <div className="flex-1 overflow-hidden">
                          ...
                          {carrierContactNotFound && (
                            <div className="rounded-lg border border-dashed p-3 text-center space-y-2">
                              ...
                              <Button type="button" size="sm" variant="outline" onClick={handleCarrierInvite} disabled={carrierInviteLoading} className="gap-2">
