...
export function LoadDetailView({ loadId, isModal, onClose }: LoadDetailViewProps) {
  ...
  const fetchLoad = useCallback(async () => {
    ...
    setError("");
    try {
      const { data } = await api.get<Load>(`/loads/${loadId}`);
      setLoad(data);

      if (TRACKABLE_STATUSES.includes(data.status)) {
        try {
          const posRes = await api.get<Position>(`/loads/${loadId}/position`);
  ...
  const handleAssign = async () => {
    if (!loadId || !selectedCarrierId) return;
    setActionError("");
    setAssignLoading(true);
    try {
      await api.post(`/loads/${loadId}/assign`, { carrier_id: selectedCarrierId });
      setAssignOpen(false);
  ...
  const handleCancel = async () => {
    ...
    try {
      await api.post(`/loads/${loadId}/cancel`);
      await fetchLoad();
    } catch (err) {
      setActionError(getApiErrorMessage(err));
    } finally {
  ...
  const handleConfirm = async () => {
    ...
    try {
      await api.post(`/loads/${loadId}/confirm`);
      await fetchLoad();
    } catch (err) {
      setActionError(getApiErrorMessage(err));
    } finally {
  ...
  const canConfirm = load.status === "dropped_off" || load.status === "completed";
  ...
  return (
    <div className={`flex flex-col ${isModal ? "h-full" : "h-[calc(100vh-4rem)]"}`}>
      {/* ── Header ── */}
      <div className={`shrink-0 border-b px-4 py-3 lg:px-6 ${isModal ? "pr-12 lg:pr-14" : ""}`}>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2 shrink-0 flex-wrap">
            {/* Open full page button — only in modal mode */}
            {isModal && (
              <Button
                variant="outline"
                size="sm"
                onClick={() => navigate(`/loads/${loadId}`)}
                className="gap-1.5"
              >
            ...
            {canAssign && (
              <Button size="sm" onClick={() => { setAssignOpen(true); fetchCarriers(); }} className="gap-1">
                <UserPlus size={14} />
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0">
        {/* ── Map ── */}
        <div className="relative h-[280px] lg:h-auto lg:flex-1 lg:order-2 lg:sticky lg:top-0 border-b lg:border-b-0 lg:border-l">
          <MapLibreTrackingMap
          ...
          <div className="absolute top-3 right-3 z-20">
            <Button
              size="sm"
              variant="secondary"
              onClick={() => setTrackingLinkOpen(true)}
              className="gap-1.5 shadow-md"
            >
              <Share2 size={14} />
              {t("load_detail_share_tracking")}
            </Button>
          </div>
        </div>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          setAssignOpen(open);
          if (!open) { setAssignCarrierSearch(""); setSelectedCarrierId(""); setAssignTab("existing"); }
        }}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t("load_detail_assign_carrier")}</DialogTitle>
            <DialogDescription>{t("load_detail_assign_dialog_desc")}</DialogDescription>
          </DialogHeader>
              ...
              <CopyLinkPanel
                ...
                onGenerate={async () => {
                  const { data } = await api.post<InviteLinkResponse>(
                    `/loads/${loadId}/invite-link`
                  );
                  return {
                    url: data.url,
                    note: data.expires_at
                      ? `${t("load_detail_invite_link_expires")} ${utcToLocalDisplay(data.expires_at)}`
                      : undefined,
                  };
                }}
              />
      ...
      <Dialog open={trackingLinkOpen} onOpenChange={setTrackingLinkOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t("load_detail_share_tracking")}</DialogTitle>
            <DialogDescription>{t("load_detail_share_tracking_desc")}</DialogDescription>
          </DialogHeader>

          <CopyLinkPanel
            generateLabel={t("load_detail_generate_tracking_link")}
            onGenerate={async () => {
              const { data } = await api.post<TrackingLinkResponse>(
                `/loads/${loadId}/tracking-link`
              );
              return { url: data.url };
            }}
          />
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setTrackingLinkOpen(false)}>
              {t("common_close")}
            </Button>
          </DialogFooter>
        </DialogContent>
